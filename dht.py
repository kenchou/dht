...
parser.add_argument('-f', '--format', help='output format. -f log will output "YYYY-MM-DD hh:mm:ss temperature,humidity')
...
data = {'datetime': now, 'temperature': temperature, 'humidity': humidity}

if args.format == 'log':
    output_format = '{datetime:%Y-%m-%d %H:%M:%S} {temperature:.1f},{humidity:.1f}'
elif args.format == 'simple':
    output_format = '{temperature:.1f},{humidity:.1f}'
elif args.format:
    output_format = args.format
else:
    output_format = '温度: {temperature:.1f}°C, 相对湿度: {humidity:.1f}%'
